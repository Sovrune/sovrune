# Public roadmap

## 0.1 — operating contract

- [x] BusinessState and Company Adapter contracts
- [x] Four provider adapters
- [x] Deterministic Acme operating loop
- [x] Command center and Docker deployment
- [ ] Configuration wizard
- [ ] Persistent decision and prediction ledger

## 0.2 — accountable execution

- [ ] Office templates and capability requirements
- [ ] Provider routing, fallbacks, and budgets
- [ ] Artifact-backed issue workflow
- [ ] Human approval queue
- [ ] Outcome measurement windows

## 0.3 — connected company

- [ ] Connector SDK and marketplace format
- [ ] Scheduled evidence collectors
- [ ] Agent fleet observability
- [ ] Structured inter-office messaging
- [ ] Slack notifications

## 1.0 — production foundation

- [ ] Authentication and role-based access
- [ ] Audit export
- [ ] Backup and upgrade tooling
- [ ] Multi-company isolation
- [ ] Threat model and external security review
